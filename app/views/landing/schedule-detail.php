<?php $title = 'Detail Praktikum - ' . e($schedule['course']); ?>
...
<div class="bg-gray-50 min-h-screen pb-12">
    <div class="bg-white border-b border-gray-200">
        <div class="max-w-screen-xl mx-auto px-4 py-4">
            <a href="<?= url('/schedule') ?>" class="inline-flex items-center text-sm font-medium text-gray-500 hover:text-primary transition-colors">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 mb-8 relative overflow-hidden">
            ...
            <div class="relative z-10">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <span class="bg-blue-100 text-primary text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wide mb-2 inline-block">
                            <?= e($schedule['program_study'] ?? 'Umum') ?>
                        </span>
                        <h1 class="text-3xl md:text-4xl font-extrabold text-gray-900 mb-2"><?= e($schedule['course']) ?></h1>
                        <div class="flex items-center text-gray-500 text-lg">
                            <i class="bi bi-geo-alt-fill mr-2 text-red-500"></i>
                            <?= e($schedule['lab_name']) ?> <span class="mx-2">•</span> <?= e($schedule['location']) ?>
                        </div>
                    </div>
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 text-center">
                    ...
                    <div class="relative w-32 h-32 mx-auto mb-4">
                        <img src="<?= !empty($schedule['lecturer_photo']) ? $schedule['lecturer_photo'] : 'https://ui-avatars.com/api/?name=' . urlencode($schedule['lecturer']) . '&background=random' ?>"
                            alt="Dosen"
                            class="w-full h-full rounded-full object-cover border-4 border-white shadow-lg">
                        <div class="absolute bottom-1 right-1 bg-blue-500 text-white p-1 rounded-full shadow-sm border-2 border-white">
                    ...
                    <h4 class="text-lg font-bold text-gray-900 mb-1"><?= e($schedule['lecturer']) ?></h4>
                    <span class="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full">Dosen</span>
                </div>

                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <h3 class="text-sm font-bold text-gray-400 uppercase tracking-wider mb-6 text-center">Tim Asisten</h3>

                    <div class="space-y-6">
                        <div class="flex items-center space-x-4">
                            <img src="<?= !empty($schedule['assistant_photo']) ? $schedule['assistant_photo'] : 'https://ui-avatars.com/api/?name=' . urlencode($schedule['assistant']) . '&background=random' ?>"
                                class="w-12 h-12 rounded-full object-cover shadow-sm border border-gray-200">
                            <div>
                                <h5 class="font-bold text-gray-900 text-sm"><?= e($schedule['assistant']) ?></h5>
                                <span class="text-xs text-primary font-medium">Asisten Utama (1)</span>
                            </div>
                        </div>

                        <?php if (!empty($schedule['assistant_2']) && $schedule['assistant_2'] !== '-'): ?>
                            <div class="flex items-center space-x-4 pt-4 border-t border-gray-50">
                                <img src="<?= !empty($schedule['assistant2_photo']) ? $schedule['assistant2_photo'] : 'https://ui-avatars.com/api/?name=' . urlencode($schedule['assistant_2']) . '&background=random' ?>"
                                    class="w-12 h-12 rounded-full object-cover shadow-sm border border-gray-200">
                                <div>
                                    <h5 class="font-bold text-gray-900 text-sm"><?= e($schedule['assistant_2']) ?></h5>
                ...
                <div class="bg-gradient-to-br from-primary to-blue-700 rounded-2xl shadow-lg p-6 text-white text-center">
