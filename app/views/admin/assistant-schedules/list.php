    ...
    <main class="p-4 sm:ml-64 pt-8 transition-all duration-300">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <div>
                    <h1 class="text-2xl font-bold tracking-tight text-slate-900">Jadwal Piket Asisten</h1>
                    <p class="text-slate-500 text-sm mt-1">Kelola jadwal harian asisten laboratorium.</p>
                </div>
            ...
            <div
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-slate-600">
                        <thead class="text-xs text-slate-500 uppercase bg-slate-50/80 border-b border-slate-100">
                            <tr>
                                <th scope="col" class="px-6 py-4 font-semibold">Nama Asisten</th>
                                <th scope="col" class="px-6 py-4 font-semibold">Hari Bertugas</th>
                            ...
                            <tr class="group hover:bg-slate-50/80 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 rounded-full bg-gradient-to-br from-primary-100 to-primary-200 flex items-center justify-center text-primary-700 font-bold text-xs border-2 border-white shadow-sm shrink-0">
                                            <?= strtoupper(substr($schedule['assistant_name'], 0, 2)) ?>
                                        </div>
                                        <div>
                                            <div class="font-semibold text-slate-900">
                                                <?= e($schedule['assistant_name']) ?></div>
                                ...
                                <td class="px-6 py-4">
                                    <?php if (in_array(strtolower($schedule['status']), ['active', 'scheduled'])): ?>
                                    <span
                                    ...
                                    <?php else: ?>
                                    <span
                                        class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-500 border border-slate-200">
                                        Inactive
                                    </span>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div
                                        ...
                                        <form method="POST"
                                            action="<?= url('/admin/assistant-schedules/' . $schedule['id'] . '/delete') ?>"
                                            class="inline">
                                            <button type="submit"
                ...
                <div
                    class="mt-auto border-t border-slate-100 bg-slate-50 px-6 py-3 text-xs text-slate-500 flex justify-between">
                    <span>Total: <?= count($schedules) ?> jadwal</span>
                </div>
                ...
                <div class="flex flex-col items-center justify-center flex-1 py-12 text-center">
                    ...
                    <p class="text-slate-500 max-w-sm mt-1 mb-6 text-sm">Belum ada asisten yang dijadwalkan piket.</p>
                    <a href="<?= url('/admin/assistant-schedules/create') ?>"
                        class="text-primary-600 hover:text-primary-700 font-medium hover:underline text-sm flex items-center gap-1">
                        <i class="bi bi-plus-circle"></i> Buat Jadwal Pertama
                    </a>
                </div>
