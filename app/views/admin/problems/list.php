...
<div class="admin-layout antialiased bg-slate-50 min-h-screen">
    ...
    <main class="p-4 sm:ml-64 pt-8 transition-all duration-300">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="mb-8">
                <h1 class="text-2xl font-bold tracking-tight text-slate-900">Problem Reports</h1>
                <p class="text-slate-500 text-sm mt-1">Monitor and manage technical issues reported across all laboratories.</p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                ...
                <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex items-center justify-between hover:shadow-md transition-shadow">
                    <div>
                        <p class="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Reports</p>
                ...
                <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex items-center justify-between hover:shadow-md transition-shadow relative overflow-hidden group">
                    <div class="absolute inset-0 bg-rose-50 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                    <div class="relative z-10">
                        <p class="text-xs font-bold text-rose-500 uppercase tracking-wider">New Reports</p>
                        <h3 class="text-2xl font-bold text-rose-600 mt-1"><?= $statistics['reported'] ?? 0 ?></h3>
                    </div>
                    <div class="relative z-10 w-10 h-10 rounded-xl bg-rose-100 text-rose-600 flex items-center justify-center">
                ...
                <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex items-center justify-between hover:shadow-md transition-shadow relative overflow-hidden group">
                    ...
                    <div class="relative z-10">
                        <p class="text-xs font-bold text-amber-500 uppercase tracking-wider">In Progress</p>
                        <h3 class="text-2xl font-bold text-amber-600 mt-1"><?= $statistics['in_progress'] ?? 0 ?></h3>
                    </div>
                    <div class="relative z-10 w-10 h-10 rounded-xl bg-amber-100 text-amber-600 flex items-center justify-center">
                ...
                <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex items-center justify-between hover:shadow-md transition-shadow relative overflow-hidden group">
                    ...
                    <div class="relative z-10">
                        <p class="text-xs font-bold text-emerald-500 uppercase tracking-wider">Resolved</p>
                        <h3 class="text-2xl font-bold text-emerald-600 mt-1"><?= $statistics['resolved'] ?? 0 ?></h3>
                    </div>
                    <div class="relative z-10 w-10 h-10 rounded-xl bg-emerald-100 text-emerald-600 flex items-center justify-center">
            ...
            <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col min-h-[600px]">
                ...
                <div class="overflow-x-auto w-full">
                    <table class="w-full text-sm text-left text-slate-600">
                        <thead class="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-100">
                            <tr>
                                <th scope="col" class="px-6 py-4 font-semibold">Location (Lab & PC)</th>
                                <th scope="col" class="px-6 py-4 font-semibold w-1/3">Issue Description</th>
                                <th scope="col" class="px-6 py-4 font-semibold">Status</th>
                                <th scope="col" class="px-6 py-4 font-semibold">Reporter</th>
                                    ...
                                    <tr class="group hover:bg-slate-50/80 transition-colors duration-200">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex items-start gap-3">
                                                <div class="w-9 h-9 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center shrink-0 border border-blue-100 mt-1">
                                                    <i class="bi bi-pc-display"></i>
                                                </div>
                                                <div>
                                                    <div class="font-bold text-slate-900"><?= e($problem['lab_name']) ?></div>
                                                    <div class="inline-flex items-center gap-1 mt-1 text-xs font-mono bg-slate-100 px-1.5 py-0.5 rounded text-slate-600 border border-slate-200">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div class="flex flex-col">
                                                    <span class="text-xs font-semibold text-slate-700"><?= e($problem['reporter_name']) ?></span>
                                                    <span class="text-[10px] text-slate-400"><?= formatDateTime($problem['reported_at']) ?></span>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            <div class="flex items-center justify-end gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200">
                                                ...
                                                <form method="POST" action="<?= url('/admin/problems/' . $problem['id'] . '/delete') ?>" onsubmit="return confirm('Are you sure you want to delete this report?')" class="inline">
                                                    <button type="submit" 
                                ...
                                <tr>
                                    <td colspan="5">
                                        <div class="flex flex-col items-center justify-center py-20 text-center">
                                            ...
                                            <h3 class="text-lg font-semibold text-slate-900">No Problems Found</h3>
                                            <?php if($currentStatus): ?>
                                                <p class="text-slate-500 max-w-sm mt-1 mb-6 text-sm">There are no reports with status "<?= ucfirst(str_replace('_', ' ', $currentStatus)) ?>".</p>
                                                <a href="<?= url('/admin/problems') ?>" class="text-primary-600 hover:underline text-sm">Clear Filter</a>
                                            <?php else: ?>
                                                <p class="text-slate-500 max-w-sm mt-1 text-sm">Great! All systems are running smoothly.</p>
                                            <?php endif; ?>
                                        </div>
                                    </td>
                                </tr>
                ...
                <div class="mt-auto border-t border-slate-100 bg-slate-50 px-6 py-3 text-xs text-slate-500 flex justify-between items-center">
                    <span>Showing <strong><?= count($problems) ?></strong> reports</span>
                    <?php if($currentStatus): ?>
                        <span class="bg-slate-200 px-2 py-0.5 rounded text-slate-600">Filter: <?= ucfirst($currentStatus) ?></span>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
