                                    ...
                                    <tr class="group hover:bg-slate-50/80 transition-colors duration-200">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-4">
                                                <div class="w-10 h-10 rounded-full bg-slate-100 border-2 border-white shadow-sm overflow-hidden flex-shrink-0 relative">
                                                    <?php if (!empty($hl['photo'])): ?>
                                                        <img src="<?= e($hl['photo']) ?>" class="w-full h-full object-cover" alt="<?= e($hl['name']) ?>">
                                                    <?php else: ?>
                                                        <div class="w-full h-full flex items-center justify-center text-primary-600 bg-primary-50 font-bold text-sm">
                                                ...
                                                <div>
                                                    ...
                                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-slate-100 text-slate-500 border border-slate-200 mt-1">
                                                        <?= e($hl['position']) ?>
                                                    </span>
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex items-start gap-2">
                                                ...
                                                <div class="max-w-[200px]">
                                                    ...
                                                    <?php if(!empty($hl['notes'])): ?>
                                                        <p class="text-xs text-slate-500 italic truncate" title="<?= e($hl['notes']) ?>">
                                                            "<?= e($hl['notes']) ?>"
                                                        </p>
                                                    <?php else: ?>
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            <div class="flex items-center justify-end gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200">
                                                ...
                                                <form method="POST" action="<?= url('/admin/head-laboran/' . $hl['id'] . '/delete') ?>" onsubmit="return confirm('Apakah anda yakin ingin menghapus staff ini?')" class="inline">
                                                    <button type="submit" 
                                ...
                                <tr>
                                    <td colspan="4">
                                        <div class="flex flex-col items-center justify-center py-20 text-center">
                                            ...
                                            <p class="text-slate-500 max-w-sm mt-1 mb-6 text-sm">Tambahkan data Kepala Laboran atau Staff untuk mulai memantau kehadiran.</p>
                                            <a href="<?= url('/admin/head-laboran/create') ?>" class="text-primary-600 hover:text-primary-700 font-medium hover:underline text-sm flex items-center gap-1">
