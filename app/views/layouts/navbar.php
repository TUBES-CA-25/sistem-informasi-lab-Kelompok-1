<nav class="bg-white/95 backdrop-blur-md border-b border-slate-200 fixed w-full z-50 top-0 start-0 transition-all duration-300">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <div class="flex items-center md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
            ...
            <?php if (isLoggedIn()): ?>
                <div class="flex items-center gap-4">
                    <div class="hidden md:block text-right">
                        <span class="block text-sm font-bold text-slate-700"><?= e(explode(' ', $_SESSION['user_name'])[0]) ?></span>
                        <span class="block text-[10px] font-bold uppercase tracking-wider text-slate-500"><?= getUserRole() ?></span>
                    </div>

                    <button type="button" class="flex text-sm bg-slate-800 rounded-full md:me-0 focus:ring-4 focus:ring-slate-300 transition-transform hover:scale-105" id="user-menu-button" aria-expanded="false" data-dropdown-toggle="user-dropdown" data-dropdown-placement="bottom">
                        <span class="sr-only">Open user menu</span>
                        <div class="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 font-bold border-2 border-slate-200 shadow-sm">
            ...
            <button data-collapse-toggle="navbar-sticky" type="button" class="inline-flex items-center p-2 w-10 h-10 justify-center text-slate-500 rounded-lg md:hidden hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-slate-200 ms-2" aria-controls="navbar-sticky" aria-expanded="false">
        ...
        <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-sticky">
            <ul class="flex flex-col p-4 md:p-0 mt-4 font-medium border border-slate-100 rounded-lg bg-slate-50 md:space-x-6 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white/0">

                <li>
                    <a href="<?= url('/') ?>" class="block py-2 px-3 rounded md:p-0 transition-colors <?= ($_SERVER['REQUEST_URI'] == '/iclabs/public/' || $_SERVER['REQUEST_URI'] == '/iclabs/public/home') ? 'text-sky-600 font-bold' : 'text-slate-600 hover:text-sky-600' ?>">
                        Home
                    </a>
                </li>
                <li>
                    <a href="<?= url('/schedule') ?>" class="block py-2 px-3 rounded md:p-0 transition-colors <?= strpos($_SERVER['REQUEST_URI'], '/schedule') !== false ? 'text-sky-600 font-bold' : 'text-slate-600 hover:text-sky-600' ?>">
                        Jadwal Lab
                    </a>
                </li>
                <li>
                    <a href="<?= url('/presence') ?>" class="block py-2 px-3 rounded md:p-0 transition-colors <?= strpos($_SERVER['REQUEST_URI'], '/presence') !== false ? 'text-sky-600 font-bold' : 'text-slate-600 hover:text-sky-600' ?>">
                        Presence
                    </a>
                </li>
